...
#define MAX_HEADER_NAME_LENGTH 72
	...
	if (multiline) {
		...
		if (sendbody) {
			...
			res = bbs_sendfile(imap->node->wfd, fileno(fp), &offset, (size_t) size); /* We must manually tell it the offset or it will be at the EOF, even with rewind() */
			bbs_node_fd_writef(imap->node, imap->node->wfd, ")\r\n"); /* And the finale (don't use imap_send for this) */
