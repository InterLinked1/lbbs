...
#define TEST_PRE_EXEC(cmd) if (system(cmd)) { bbs_error("system(%s) failed: %s\n", cmd, strerror(errno)); return -1; }
...
/* This domain is statically routed to localhost but not an accepted domain for incoming mail.
 * Useful for testing permanent failure behavior. */
#define TEST_EMAIL_EXTERNAL_OUTGOING_BOUNCE_DOMAIN "externalbounce.example.com"
#define TEST_EMAIL_EXTERNAL_OUTGOING_BOUNCE "test@" TEST_EMAIL_EXTERNAL_OUTGOING_BOUNCE_DOMAIN

/* Aliased to TEST_USER in mod_mail.conf */
#define TEST_EMAIL_EXTERNAL_OUTGOING_TEMPFAIL_DOMAIN "tempfail.example.com"
#define TEST_EMAIL_EXTERNAL_OUTGOING_TEMPFAIL "tempfail@" TEST_EMAIL_EXTERNAL_OUTGOING_TEMPFAIL_DOMAIN
...
#define TEST_SASL3 "dGVzdHVzZXIzAHRlc3R1c2VyMwBQQHNzdzByRA=="
